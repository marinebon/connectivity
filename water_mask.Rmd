...
for (rgn in regions){
  ...
  tif_m_9km = sprintf('P:/habitats/%s-9km_mask.tif'   , rgn)
  tif_p_9km = sprintf('P:/habitats/%s-9km_patchid.tif', rgn)
  ...
  writeRaster(r_m, tif_m, overwrite=T, dataType = 'INT1U')

  # generate patch IDs raster
  r_p = setValues(r_m, 1:ncell(r_m)) %>%
    mask(r_m)
  writeRaster(r_p, tif_p, overwrite=T, dataType = 'INT4S')
  
  # reduce resolution, ie increase pixel size to 9 km
  r_m_9km = aggregate(r_m, fact_9km)
  writeRaster(r_m_9km, tif_m_9km, overwrite=T, dataType = 'INT1U') 
  
  # generate patch IDs raster
  r_p_9km = setValues(r_m_9km, 1:ncell(r_m_9km)) %>%
    mask(r_m_9km)
  writeRaster(r_p_9km, tif_p_9km, overwrite=T, dataType = 'INT4S')
