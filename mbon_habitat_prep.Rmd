...
```{r raster formats}
# try other formats for ArcGIS ----
r_m = raster(tif_all)
for (ext in c('grd','asc','sdat','rst','nc','tif','envi','bil','img')){ # ext='grd'
  out = sprintf('%s.%s', tools::file_path_sans_ext(tif_all), ext)
  writeRaster(r_m, out)
}
```
...
```{r}
r = raster(tif_all)
mbnms_buf = readOGR('H:/MBON/study_area.gdb', 'mbnms_500buf_mol')
tif_mbnms = file.path(dir_hab, 'hab_id_mbnms_500buf.tif')
fknms_buf = readOGR('H:/MBON/study_area.gdb', 'fknms_500buf_mol')
tif_fknms = file.path(dir_hab, 'hab_id_fknms_500buf.tif')

r_mbnms_buf = rasterize(mbnms_buf, r)
r_mbnms_buf_t = trim(r_mbnms_buf)
r_fknms_buf = rasterize(fknms_buf, r)
r_fknms_buf_t = trim(r_fknms_buf)

# getting Error: cannot allocate vector of size 2.8 Gb
mapview(r_mbnms_buf)
mapview(r_fknms_buf)


r_mbnms = crop(r, fknms_buf)
writeRaster(r_mbnms, tif_mbnms, overwrite=T)
r_fknms = crop(r,fknms_buf)
writeRaster(r_fknms, tif_fknms, overwrite=T)
...
#aggregate up to raster cell size
...
# - get digit in eth place (eg 2 for 2e2, ie 3 for 4321) using division modulus and remainder: 4321 %/% 1e2 %% 10 = 3
